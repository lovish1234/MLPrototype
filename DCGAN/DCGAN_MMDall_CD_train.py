...
import sys
sys.path.append('../')
from Datasets.loadDataset import loadDataset, getHelperClass, getEpochs, getChannels, getClasses, checkAndCreateFolder
    ...
    for epoch in range(primaryEpochs):
        for i, data in enumerate(primaryTrainLoader, 0):
            ...
            if (i==0) and epoch==(primaryEpochs-1) :
                ...
                modelFolder = resultDir+'models/crossDataSetMMDall'+'/'+folder+'/'
                plotFolder = resultDir+'plots/crossDataSetMMDall'+'/'+folder+'/'
                
                checkAndCreateFolder(modelFolder)
                checkAndCreateFolder(plotFolder)
                
                modelFileName = modelFolder+fileName+'_'+str(epoch)+'.pt'
                plotFileName = plotFolder+fileName+'_'+str(epoch)+'.png'
    ...
    for cls in primaryClasses:
        for instance in primaryInstances:
            for helperInstance in helperInstances:
                ...
                x = loadDataset(primaryDataSet, cls, instance, except_=0)
                y = loadDataset(helperDataSet, helperClass, helperInstance, except_=2)
...
if __name__=='__main__':
    ...
    trainSamples(primaryDataSet, helperDataSet, primaryClasses, primaryInstances, helperInstances)
