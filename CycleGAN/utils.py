...
def plot_train_result(real_image, gen_image, recon_image, epoch, save=False, save_dir='results/', show=False, fig_size=(5, 5)):
    ...
    for ax, img in zip(axes.flatten(), imgs):
        ...
        img = (((img - img.min()) * 255) / (img.max() -
            img.min())).astype(np.uint8)
...
def plot_test_result(real_image, gen_image, recon_image, index, save=False, save_dir='results/', show=False):
    ...
    #print (real_image.size(0), real_image.size(1), real_image.size(2),
    #     real_image.size(3))

    #fig_size = (real_image.size(2) * 3 / 100, 3 *real_image.size(3) / 100)
    #fig, axes = plt.subplots(1, 3, figsize=fig_size)
    batchSize = real_image.size(0)
    fig, axes = plt.subplots(batchSize, 3)
    fig.tight_layout()
    imgs =[]
    for batchImage in range(batchSize):
        imgs.extend((to_np(real_image[batchImage]),
                     to_np(gen_image[batchImage]),
                     to_np(recon_image[batchImage])))
    for ax, img in zip(axes.flatten(), imgs):
        ax.axis('off')
        ax.set_adjustable('box-forced')
        # Scale to 0-255
    ...
    if save:
        ...
        fig.subplots_adjust(bottom=None)
        fig.subplots_adjust(top=None)
