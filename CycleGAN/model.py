...
class ConvBlock(torch.nn.Module):
    def __init__(self, input_size, output_size, kernel_size=3, stride=2, padding=1, activation='relu', batch_norm=True):
        ...
        # weather to use batch_norm or not
...
class Generator(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if debug:
            print ("Size of Gen after enc1 : ",enc1.data.shape)

        enc2 = self.conv2(enc1)
        if debug:
            print ("Size of Gen after enc2 : ",enc2.data.shape)

        enc3 = self.conv3(enc2)
        if debug:
            print ("Size of Gen after enc3 : ",enc3.data.shape)

        # Resnet blocks
        res = self.resnet_blocks(enc3)
        if debug:
            print ("Size of Gen after resnet : ",res.data.shape)

        # Decoder
        dec1 = self.deconv1(res)
        if debug:
            print ("Size of Gen after dec1 : ",dec1.data.shape)

        dec2 = self.deconv2(dec1)
        if debug:
            print ("Size of Gen after dec2 : ",dec2.data.shape)

        out = self.deconv3(self.pad(dec2))
        if debug:
            print ("Size of Gen after dec3 : ",out.data.shape)
        return out
...
class Discriminator(torch.nn.Module):
    ...
    def forward(self, x):
        if debug:
            print ("Size of Dis after conv blocks : ",x.data.shape)
        out = self.conv_blocks(x)
        if debug:
            print ("Size of Dis after conv blocks : ",out.data.shape)
